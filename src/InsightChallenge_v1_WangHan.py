...
class BEAnalytics(object):
        ...
        def getTopHost(self,logList):
                ...
                hostIdx = {}
                for log in logList:
                        if log.host not in hostCnt:
                                hostCnt[log.host] = 0
                        hostCnt[log.host]+=1
                        if log.host not in hostIdx:
                                hostIdx[log.host] = log.key
                topHost = self.topKFrequent(hostCnt,10)
                topHostWIdx = [(host, cnt, hostIdx[host]) for (host, cnt) in topHost]
                topHostWIdx.sort(key = operator.itemgetter(2))
                topHostWIdx.sort(key = operator.itemgetter(1), reverse=True)
                
                outputs = []
                for host in topHostWIdx:
        ...
        def getTopResource(self,logList):
                resCnt = OrderedDict()
                for log in logList:
                        if log.resource not in resCnt:
                                resCnt[log.resource] = 0
